 ...
 */
ScalarWidget::ScalarWidget(const char* _name, GLMotif::Container* _parent, Volume* _volume, int _component, bool _manageChild) :
    GLMotif::Widget(_name, _parent, false), currentGaussian(-1), gaussian(false), dragging(false), numberOfGaussians(0),
            unselected(false), volume(_volume) {
...
void ScalarWidget::calculateHistogram(void) {
    int numberOfVoxels = volume->getNumberOfVoxels();
    if (component == ALPHA_COMPONENT) {
        for (int i = 0; i < numberOfVoxels; i++) {
            alphaHistogram[volume->getVoxel(i)] += 1.0f;
        }
        float _maximum = 0;
        for (int i = 0; i < numberOfEntries; i++) {
            alphaHistogram[i] = float(log(double(alphaHistogram[i])));
            if (_maximum < alphaHistogram[i])
                _maximum = alphaHistogram[i];
        }
        for (int i = 0; i < numberOfEntries; i++) {
            alphaHistogram[i] /= _maximum;
        }
    } else {
        for (int i = 0; i < numberOfVoxels; i++) {
            redHistogram[volume->getRedVoxel(i)] += 1.0f;
            greenHistogram[volume->getGreenVoxel(i)] += 1.0f;
            blueHistogram[volume->getBlueVoxel(i)] += 1.0f;
        }
        float _maximum = 0;
        for (int i = 0; i < numberOfEntries; i++) {
            redHistogram[i] = float(log(double(redHistogram[i])));
            if (_maximum < redHistogram[i])
                _maximum = redHistogram[i];
            greenHistogram[i] = float(log(double(greenHistogram[i])));
            if (_maximum < greenHistogram[i])
                _maximum = greenHistogram[i];
            blueHistogram[i] = float(log(double(redHistogram[i])));
            if (_maximum < blueHistogram[i])
                _maximum = blueHistogram[i];
        }
        for (int i = 0; i < numberOfEntries; i++) {
            redHistogram[i] /= _maximum;
            greenHistogram[i] /= _maximum;
            blueHistogram[i] /= _maximum;
        }
    }
...
unsigned char ScalarWidget::clamp(int x, unsigned char a, unsigned char b) const {
 ...
 */
void ScalarWidget::exportScalar(unsigned char* colormap) const {
    if (!gaussian) {
        for (int i = 0; i < numberOfEntries; ++i) {
            ...
            GLfloat w1 = GLfloat((nextControlPoint->getValue() - value) / (nextControlPoint->getValue()
                    - previousControlPoint->getValue()));
            colormap[4 * (i * 256 + i) + component] = (unsigned char) ((previousControlPoint->getScalar() * w1
                    + nextControlPoint->getScalar() * w2) * 255.0f);
        }
    } else {
        for (int i = 0; i < 256; i++)
            colormap[4 * (i * 256 + i) + component] = (unsigned char) (opacities[i] * 255.0f);
    }
} // end exportScalar()
 ...
 * parameter component - int
 */
void ScalarWidget::exportScalar(unsigned char* colormap, int component) {
    saveState();
    updatePointers(component);
    if (!gaussian) {
        for (int i = 0; i < numberOfEntries; ++i) {
...
/*
 * load - Load scalar control points from file.
 *
 * parameter fileName - const char*
 */
void ScalarWidget::load(const char* fileName) {
    std::string buffer;
    std::ifstream scalarFile(fileName);
    deleteControlPoints();
    int numberOfControlPoints = 0;
    while (!scalarFile.eof()) {
        getline(scalarFile, buffer);
        if (buffer[0] != '#') {
            std::string bufferString = buffer;
            StringTokenizer stringTokenizer(bufferString, " ");
            if (stringTokenizer.getNumberOfTokens() == 5) {
                double _value = stringTokenizer.nextDoubleToken();
                float _scalar = stringTokenizer.nextFloatToken();
                if (numberOfControlPoints == 0) {
                    first->setValue(_value);
                    first->setScalar(_scalar);
                } else {
                    if (numberOfControlPoints > 1) {
                        ScalarWidgetControlPoint* _controlPoint = new ScalarWidgetControlPoint(last->getValue(), last->getScalar());
                        _controlPoint->left = last->left;
                        _controlPoint->left->right = _controlPoint;
                        _controlPoint->right = last;
                        last->left = _controlPoint;
                    }
                    last->setValue(_value);
                    last->setScalar(_scalar);
                }
                ++numberOfControlPoints;
            }
        }
 ...
 * parameter fileName - const char*
 */
void ScalarWidget::save(const char* fileName) const {
    std::ofstream scalarFile(fileName, std::ios::out);
    for (ScalarWidgetControlPoint* controlPointPtr = first; controlPointPtr != 0; controlPointPtr = controlPointPtr->right)
        scalarFile << controlPointPtr->getValue() << " " << controlPointPtr->getScalar() << std::endl;
    scalarFile.close();
} // end save()
