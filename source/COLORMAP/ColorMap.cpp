...
void ColorMap::createColorMap(int colormap) {
	...
	updateControlPoints();
	ColorMapChangedCallbackData callbackData(this);
	colorMapChangedCallbacks.call(&callbackData);
}
 ...
 */
void ColorMap::exportColorMap(unsigned char* colormap) const {
	...
	for (int i=0; i<numberOfEntries; ++i) {
		...
		for (int j=0; j<3; ++j)
			colormap[4*(i*256+i)+j]=(unsigned char)((previousControlPoint->rgbaColor->getValues(j)*w1
					+nextControlPoint->rgbaColor->getValues(j)*w2)*255.0f);
	}
}
...
/*
 * loadColorMap - Load color map from file.
 * 
 * parameter fileName - const char*
 */
void ColorMap::loadColorMap(const char* fileName) {
	std::string buffer;
	std::ifstream colorMapFile(fileName);
	deleteColorMap();
	int numberOfControlPoints=0;
	while (!colorMapFile.eof()) {
		getline(colorMapFile, buffer);
		if (buffer[0]!='#') {
			std::string bufferString = buffer;
			StringTokenizer stringTokenizer(bufferString, " ");
			if (stringTokenizer.getNumberOfTokens()==5) {
				double value = stringTokenizer.nextDoubleToken();
				RGBAColor *rgbaColor;
				float r = stringTokenizer.nextFloatToken();
				float g = stringTokenizer.nextFloatToken();
				float b = stringTokenizer.nextFloatToken();
				float a = stringTokenizer.nextFloatToken();
				a = 0.0f;
				rgbaColor=new RGBAColor(r,g,b,a);
				if (numberOfControlPoints==0) {
					first->value=value;
					first->rgbaColor=rgbaColor;
				} else {
					if (numberOfControlPoints>1) {
						ControlPoint* _controlPoint=new ControlPoint(last->value,last->rgbaColor);
...
/*
 * saveColorMap - Save color map.
 * 
 * parameter fileName - const char*
 */
void ColorMap::saveColorMap(const char* fileName) const {
	std::ofstream colorMapFile(fileName, std::ios::out);
	for (ControlPoint* controlPointPtr=first; controlPointPtr!=0; controlPointPtr=controlPointPtr->right)
		colorMapFile << controlPointPtr->value << " " << controlPointPtr->rgbaColor->getValues(0) << " "
				<< controlPointPtr->rgbaColor->getValues(1) << " " << controlPointPtr->rgbaColor->getValues(2) << " "
				<< controlPointPtr->rgbaColor->getValues(3) << std::endl;
	colorMapFile.close();
}
